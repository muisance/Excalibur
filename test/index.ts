...
describe("Protocol", function () {
    ...
    before(async function () {
        ...
        avatarContract = await ethers
            .getContractFactory("SoulUpgradable")
            .then((Contract) =>
                upgrades.deployProxy(Contract, [hubContract.address], {
                    // https://docs.openzeppelin.com/upgrades-plugins/1.x/api-hardhat-upgrades#common-options
                    kind: "uups",
                    timeout: 120000,
                })
    ...
    describe("Config", function () {
        it("Should be owned by deployer", async function () {
            expect(await configContract.owner()).to.equal(
    ...
    describe("Avatar", function () {
        it("Should inherit protocol owner", async function () {
            expect(await avatarContract.owner()).to.equal(
                await owner.getAddress()
            );
        });
        ...
        it("Allow Multiple Owner Accounts per Avatar", async function () {
            ...
            //Expected Event
            await expect(tx)
                .to.emit(avatarContract, "Transfer")
                .withArgs(ZERO_ADDR, miscAddr, tokenId);
        ...
        it("Can add other people", async function () {
            unOwnedTokenId = await avatarContract
                .connect(tester)
                .callStatic.add(test_uri);
        ...
        it("Can update token's metadata", async function () {
            ...
            //Check URI
            expect(await avatarContract.connect(tester).tokenURI(1)).to.equal(
                test_uri
            );
        });
    ...
    describe("Action Repository", function () {
        it("Should store Actions", async function () {
            ...
            //Expect Added Event
            await expect(tx)
                .to.emit(actionContract, "ActionAdded")
                .withArgs(
                    1,
                    actionGUID,
                    action.subject,
            ...
            expect(await actionContract.actionGetURI(actionGUID)).to.equal(
                test_uri
            );
    ...
    describe("Jurisdiction", function () {
        before(async function () {
            ...
            let JAddr = await hubContract.callStatic.jurisdictionMake(
                "Test Jurisdiction",
                test_uri
            );
            ...
            let tx = await hubContract.jurisdictionMake(
                "Test Jurisdiction",
                test_uri
            );
        ...
        it("Users can join as a member", async function () {
            //Check Before
            expect(
                await this.jurisdictionContract.roleHas(
                    this.testerAddr,
                    "member"
                )
            ).to.equal(false);
        ...
        it("Role Should Track Avatar Owner", async function () {
            ...
            let tx = await avatarContract.transferFrom(
                this.tester5Addr,
                this.tester3Addr,
                tokenId
            );
            ...
            await expect(tx)
                .to.emit(avatarContract, "Transfer")
                .withArgs(this.tester5Addr, this.tester3Addr, tokenId);
            //Expect Change of Ownership
            expect(await avatarContract.ownerOf(tokenId)).to.equal(
                this.tester3Addr
            );
            //Check Membership
            expect(
            ...
            await expect(
                ...
                "ERC1155Tracker: requested account not found on source contract"
            );
        });
        ...
        it("Owner can appoint Admin", async function () {
            //Check Before
            expect(
                await this.jurisdictionContract.roleHas(this.adminAddr, "admin")
            ).to.equal(false);
            //Should Fail - Require Permissions
            await expect(
                this.jurisdictionContract
                    .connect(tester)
                    .roleAssign(this.adminAddr, "admin")
        ...
        it("Admin can appoint judge", async function () {
            //Check Before
            expect(
                await this.jurisdictionContract.roleHas(this.judgeAddr, "judge")
            ).to.equal(false);
            //Should Fail - Require Permissions
            await expect(
                this.jurisdictionContract
                    .connect(tester2)
                    .roleAssign(this.judgeAddr, "judge")
            ).to.be.revertedWith("INVALID_PERMISSIONS");
            //Assign Judge
            await this.jurisdictionContract
                .connect(admin)
                .roleAssign(this.judgeAddr, "judge");
            //Check After
            expect(
        ...
        it("Admin can Assign Roles to Lost-Souls", async function () {
            //Check Before
            expect(
                await this.jurisdictionContract.roleHasByToken(
                    unOwnedTokenId,
                    "judge"
                )
            ).to.equal(false);
            ...
            await this.jurisdictionContract
                .connect(admin)
                .roleAssignToToken(unOwnedTokenId, "judge");
            //Check After
            expect(
                await this.jurisdictionContract.roleHasByToken(
                    unOwnedTokenId,
        ...
        it("Can change Roles (Promote / Demote)", async function () {
            ...
            //Upgrade to Admin
            await this.jurisdictionContract.roleChange(
                this.tester4Addr,
                "member",
                "admin"
            );
        ...
        it("Should store Rules", async function () {
            ...
            //Add Rule
            let tx = await jurisdictionContract
                .connect(admin)
                .ruleAdd(rule, confirmation, effects1);
            ...
            //Expect Event
            await expect(tx)
                .to.emit(jurisdictionContract, "Rule")
                .withArgs(
                    1,
                    rule.about,
                    rule.affected,
            ...
            for (let effect of effects1) {
                await expect(tx)
                    .to.emit(jurisdictionContract, "RuleEffect")
                    .withArgs(1, effect.direction, effect.value, effect.name);
            }
            await expect(tx)
                .to.emit(jurisdictionContract, "Confirmation")
                .withArgs(
                    1,
                    confirmation.ruling,
            ...
            //Add Another Rule
            let tx2 = await jurisdictionContract
                .connect(admin)
                .ruleAdd(rule2, confirmation, effects2);
        ...
        it("Should Update Rule", async function () {
            let actionGUID =
                "0xa7440c99ff5cd38fc9e0bff1d6dbf583cc757a83a3424bdc4f5fd6021a2e90e2";
            let rule = {
                about: actionGUID, //"0xa7440c99ff5cd38fc9e0bff1d6dbf583cc757a83a3424bdc4f5fd6021a2e90e2",
                affected: "god", //Plaintiff / Beneficiary
                uri: "ADDITIONAL_DATA_URI",
        ...
        it("Should Update Token URI", async function () {
            ...
            await expect(
                jurisdictionContract
                    .connect(tester3)
                    .setRoleURI("admin", test_uri)
            ).to.be.revertedWith("INVALID_PERMISSIONS");
            //Set Admin Token URI
            await jurisdictionContract
                .connect(admin)
                .setRoleURI("admin", test_uri);
            //Validate
            expect(await jurisdictionContract.roleURI("admin")).to.equal(
                test_uri
            );
        });
    }); //Jurisdiction
    ...
    describe("Case", function () {
        it("Should be Created (by Jurisdiction)", async function () {
            //Soul Tokens
            let adminToken = await avatarContract.tokenByAddress(
                this.adminAddr
            );
            let tester2Token = await avatarContract.tokenByAddress(
                this.tester2Addr
            );
            ...
            let caseAddr = await jurisdictionContract
                .connect(admin)
                .callStatic.caseMake(
                    caseName,
                    test_uri,
            ...
            let tx = await jurisdictionContract
                .connect(admin)
                .caseMake(caseName, test_uri, ruleRefArr, roleRefArr, posts);
            //Expect Valid Address
            expect(caseAddr).to.be.properAddress;
            //Init Case Contract
            this.caseContract = await ethers
                .getContractFactory("CaseUpgradable")
                .then((res) => res.attach(caseAddr));
            //Expect Case Created Event
            await expect(tx)
                .to.emit(jurisdictionContract, "CaseCreated")
                .withArgs(1, caseAddr);
            //Expect Post Event
            await expect(tx)
                .to.emit(this.caseContract, "Post")
                .withArgs(
                    this.adminAddr,
                    posts[0].tokenId,
                    posts[0].entRole,
        ...
        it("Should be Created & Opened (by Jurisdiction)", async function () {
            ...
            let tester3Token = await avatarContract.tokenByAddress(
                this.tester3Addr
            );
        ...
        it("Should Update", async function () {
            let testCaseContract = await ethers
                .getContractFactory("CaseUpgradable")
                .then((res) => res.deploy());
        ...
        it("Should Add Rules", async function () {
            ...
            await this.caseContract
                .connect(admin)
                .ruleAdd(ruleRef.jurisdiction, ruleRef.id);
        });
        ...
        it("Should Write a Post", async function () {
            ...
            await expect(
                ...
                this.caseContract
                    .connect(tester)
                    .post(post.entRole, post.tokenId, post.uri)
            ).to.be.revertedWith("SOUL:NOT_YOURS");

            //Successful Post
            let tx = await this.caseContract
                .connect(tester2)
                .post(post.entRole, post.tokenId, post.uri);
            // wait until the transaction is mined
            await tx.wait();
        ...
        it("Should Assign Witness", async function () {
            ...
            await this.caseContract
                .connect(admin)
                .roleAssign(this.tester3Addr, "witness");
            //Validate
            expect(
                await this.caseContract.roleHas(this.tester3Addr, "witness")
            ).to.equal(true);
        });
        ...
        it("Jurisdiction Judges Can Assign Themselves", async function () {
            ...
            jurisdictionContract
                .connect(admin)
                .roleAssign(this.tester4Addr, "judge");
            //Assign Case Judge
            await this.caseContract
                .connect(tester4)
                .roleAssign(this.tester4Addr, "judge");
            //Validate
            expect(
                await this.caseContract.roleHas(this.tester4Addr, "judge")
            ).to.equal(true);
        });
        ...
        it("Should Validate Judge with parent jurisdiction", async function () {
            ...
            await expect(
                this.caseContract
                    .connect(admin)
                    .roleAssign(this.tester3Addr, "judge")
            ).to.be.revertedWith(
                "User Required to hold same role in Jurisdiction"
            );
        });
        ...
        it("Should Accept a Judge From the parent jurisdiction", async function () {
            ...
            //Check After
            expect(
                await this.caseContract.roleHas(this.judgeAddr, "judge")
            ).to.equal(true);
        });
        ...
        it("Should Accept Verdict URI & Close Case", async function () {
            ...
            await expect(tx)
                .to.emit(this.caseContract, "Verdict")
                .withArgs(test_uri, this.judgeAddr);
            //Expect State Event
